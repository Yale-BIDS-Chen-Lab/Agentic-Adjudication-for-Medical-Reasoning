#!/usr/bin/env python3
"""Minimal BM25 QA baseline over the shared MedRAG corpus layout."""
...
def run(args: argparse.Namespace) -> None:
    """Run BM25 retrieval + one answer pass on one JSONL split."""
    ...
    run_dir = make_run_dir(output_root, method="bm25", dataset=dataset, split=split, model=args.model)
    ...
    for idx, sample in enumerate(samples, start=1):
        ...
        try:
            # Match the reference MedRAG baseline: retrieve from the question text.
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the BM25 medical QA baseline.")
